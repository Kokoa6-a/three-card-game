...
window.OnlineSupabase = {
  ...
  async revealDoubleColor({ game_id }) {
    await ensureAnonSession();
    return await invokeFn("reveal-double-color", { game_id });
  },
  async beginExchange({ game_id }) {
    await ensureAnonSession();
    return await invokeFn("begin-exchange", { game_id });
  },
  async submitExchange({ game_id, card_id }) {
    await ensureAnonSession();
    return await invokeFn("submit-exchange", { game_id, card_id });
  },
  async beginDiscard({ game_id }) {
    await ensureAnonSession();
    return await invokeFn("begin-discard", { game_id });
  },
  async submitDiscard({ game_id, card_id }) {
    await ensureAnonSession();
    return await invokeFn("submit-discard", { game_id, card_id });
  },
  async submitDraw({ game_id, color }) {
    await ensureAnonSession();
    return await invokeFn("submit-draw", { game_id, color });
  },
  async submitJoker({ game_id, action, color }) {
    await ensureAnonSession();
    return await invokeFn("submit-joker", { game_id, action, color });
  },
  async beginPlacement({ game_id }) {
    await ensureAnonSession();
    return await invokeFn("begin-placement", { game_id });
  },
  async submitPlacement({ game_id, bets }) {
    await ensureAnonSession();
    return await invokeFn("submit-placement", { game_id, bets });
  },
  async revealField({ game_id }) {
    await ensureAnonSession();
    return await invokeFn("reveal-field", { game_id });
  },
  async computeScoringOnline({ game_id }) {
    await ensureAnonSession();
    return await invokeFn("compute-scoring", { game_id });
  },
  async nextRound({ game_id }) {
    await ensureAnonSession();
    return await invokeFn("next-round", { game_id });
  },
  unsubscribeGame
};
